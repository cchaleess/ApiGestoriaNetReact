...
function EmployeePage({ DataUserLogin }) {
  ...
  const GetEmployees = async () => {
    await new EmployeeService().getEmployeeList().then((response) => {
  ...
  const InsertEmployee = async () => {
    empSelected.departmentId = parseInt(
      document.getElementById("comboDepartamento").value
    );
    await new EmployeeService().insertEmployee(empSelected).then((response) => {
      if (response && response != null) {
  ...
  const UpdateEmployee = async () => {
    await new EmployeeService().updateEmployee(empSelected).then((response) => {
      if (response && response != null) {
        modalOnOffEdit();
        GetEmployees();
      }
  ...
  const GetDepartments = async () => {
    await new DepartmentService().getDepartmentList().then((response) => {
      if (response && response != null) {
  ...
  const seleccionaEmp = (accion) => (event) => {
    ...
    empSelected.name = event.target.parentNode.parentNode.innerHTML
      .split("td")[3]
      .replace("</", "")
      .replace(">", "");
    empSelected.departmentId = parseInt(
